...
//--------------------------------------------------
//
// Description : As incoming data may have a transfer size smaller the the
...
//               
// We clock data from there to a command FIFO external to this block using
// "wr_en" when the holding register is full of valid data or the transaction
// terminates as long as the FIFO has space. Because the FIFO is the width of
...
// upconverter. Only one of the "wr_en" signals is pulsed during burst for
// normal writes.
//               
//--------------------------------------------------
...
module DWC_UpConv_Wchan_WriteDataFifoCtrl #
  ...
  // assign mask_addr_msb = ~(size_one_hot - 1);
	...
	// if WRAP burst needs to be split into two transactions, increments the pkt counter when the upconverter receives enough data to provide a full transacton to the slave otherwise wait for WLAST.
	...
	always @(posedge ACLK or posedge sysReset) begin
		if (sysReset) begin
			...
			incr_pkt_cnt                       <= incr_pkt_cnt_next; // force to 0 when no transactions are stored in the command FIFO
			...
			always @(posedge ACLK or posedge sysReset) begin
				if (sysReset) begin
					...
					if (data_fifo_nearly_full) begin
						...
						wr_en[i] <= en_write & wr_en_words[i] & (hold_data_valid_reg | write_top_hold_reg | reached_wrap_boundary); // write data to the data FIFO when the memory is not full, the AXI data can be accepted and the holding register is full
					end
				end
			end
		end
