...
//
// Description : 
// This modules manages the signals of the write data channel within the up
// converter.
// It includes:
// - master command FIFO: it stores the relevant field of the master address
// channel   
// - slave command FIFO: it stores the relevant field of the slave address
// channel
// - data FIFO: it stores the master data, user and strobes
...
// some FIFO signals at the read side (i.e. pass_data and read enable).
//               
//
//--------------------------------------------------
...
module DWC_UpConv_WChannel #
  ...
  wire data_fifo_nearly_empty;
	...
	assign cmd_fifo_full = ( wr_cmd_slv_fifo_full | wr_cmd_mst_fifo_full ); // Both 'fifo_full' statements are really fifo nearly full
  ...
  (
    ...
    // Inputs to be decoded (from Master Cmd Fifo) - going into this holding reg
  ...
  DWC_UpConv_Wchan_WriteDataFifoCtrl #
    (
      .DATA_WIDTH_IN   ( DATA_WIDTH_IN ),
      .USER_WIDTH      ( USER_WIDTH ),
      .DATA_WIDTH_OUT  ( DATA_WIDTH_OUT )
  ...
  DWC_UpConv_WChan_ReadDataFifoCtrl # 
   (
  ...
  wr_sys_ctrl 
   (
      ...
      .sysReset           ( sysReset ),
      .fifo_nearly_empty  ( data_fifo_nearly_empty ), // Hold off trandmitting data if level of data in FIFO is below this threshold
      .fifo_empty         ( data_fifo_empty ),
      .size            ( awsize_out_mst_hold ),              // from hold
      .incr_pkt_cnt       ( incr_pkt_cnt ),
